...
5. Expand investigation only when candidates conflict, target evidence is missing, the title qualifier is not accounted for, or the verifier blocks.
...
- If a local title contains a distinct qualifier, do not map it to a Bangumi subject whose title only matches the franchise root or an earlier season. Search the exact title, inspect related subjects, or fail closed if the matching subject cannot be exposed.
- Numbered special files such as `SP01`, `SP02`, `Special 1`, or `S00E01` are candidate special entries, not automatic extras. First check the selected same-season Bangumi subject for matching special rows by sort/order. Only exclude them when those rows are absent or contradictory.
...
- For a movie/special collection from one franchise, make a local title checklist first. After one anchor search exposes the franchise, prefer the Bangumi relation graph as the series map; only direct-search individual movie titles that are still missing or conflicting after checking the graph.
- When each movie-box file has a distinctive movie title and a plausible one-movie Bangumi subject, draft exact-path movie rules with `subject_id` and `media_kind: "movie"` and validate. Do not fetch the episode list for every one-movie subject unless validation blocks or the subject has multiple rows.
...
- If the same franchise/title words keep appearing in searches but the package structure is already clear, stop searching and draft the grouped recipe. Use validation to reveal concrete missing target evidence.
- Validate a partial grouped recipe as soon as the main groups are supportable. If validation reports only missing target episodes for one group, fix that group instead of restarting the whole investigation.
- If one short or bonus-like group remains ambiguous after a clean direct title search and the relevant Bangumi anime/video relation frontier is exhausted, cover that group with `disposition: "non_bangumi_or_supplemental"` and a clear reason, then validate the whole recipe instead of spending the remaining budget on repeated broad searches. If exact Bangumi subject/episode evidence exists, map it and validate; frontier exhaustion is not required for a positive mapping.
- For companion extras such as recording diaries, interviews, cast/staff talks, travel/location features, making-of, stage greetings, memorial clips, or short bonus documentaries, do not exhaust the whole franchise relation graph. After the main subject is anchored, run one exact title search or one representative targeted lookup; if that does not expose a plausible anime/video target, validate a supplemental rule for the group.
...
- If time is running out and a large box still has unresolved groups, either submit a recipe that covers all supportable groups and marks genuinely unsupported bonus-like groups with `disposition: "non_bangumi_or_supplemental"`, or call `fail_closed` with the unresolved group names. Do not leave the case without `submit_organize_recipe_params`, `submit_organize_recipe`, or `fail_closed`.
- Do not leave the case without `submit_organize_recipe` or `fail_closed`.
...
- Missing Bangumi duration is also not negative evidence. Do not reject an otherwise supported target only because the Bangumi episode row lacks runtime metadata.
...
- If one exact visible file appears to cover multiple exposed episodes, do not degrade it to the first episode just to pass validation. Use recipe params with `source_unit: "single_file_multi_episode"`, one `exact_paths` entry, and an `episode_range`; validate so Python can check chapter or duration support. If chapter/duration support is absent or contradictory, use `fail_closed`.
- When duration is unavailable but title and numbering are coherent, draft and validate a compact recipe instead of researching only to find runtime data. Continue investigating or fail closed only when title, subject, and episode evidence are also insufficient or contradictory.
...
- `SP`, `Special`, `TVSP`: special-like content. It can be meaningful S00-style content or low-value packaging; do not discard without evidence.
...
- `Omake`, `Extra`, `Bonus`: supplemental-looking content. Map only when Bangumi evidence makes it meaningful; otherwise exclude or fail closed.
...
- For movie-box filenames, `#01` / `#02` / `#03` usually orders the package. Do not map by counting alone; read the movie subtitle or named entry and verify it against Bangumi subjects/episodes.
...
- Roman numerals such as `II`, `III`, `IV`, `V`, and `XV` are ambiguous. They may be a season/title marker, disc or volume marker, special marker, or bonus label; do not treat them as episode numbers without supporting evidence. If a Roman-numeral file would duplicate a numbered SP/OVA/Movie target that is already covered by another file, do not map it to that same target just to use the number.
- If the visible files look like `01` to `13`, make one TV episode rule for those files. If there are also files in folders like `SPs`, `Specials`, or `Extras`, handle them after the TV rule: first check numbered `SP01`-style files against Bangumi special episodes for the same subject; then check non-numbered or title-only extras separately. Do not group numbered SP files together with vague Roman-numeral or bonus files unless the numbered files also lack a clear Bangumi target. If there is no clear Bangumi entry and the file looks like package bonus material, cover it with `disposition: "non_bangumi_or_supplemental"` and explain why; if you cannot tell, validate a draft with the uncertain file using `disposition: "non_bangumi_or_supplemental"` before spending more time searching; use `fail_closed` only when that draft is not supportable or the verifier blocks.
...
- `Part A/B`, `Part 1/2`, `CD1/CD2`, and `Disc1/Disc2` can be split files for one long item. Do not automatically map each part to a separate Bangumi episode.
...
- In a `source_pattern`, replace changing CRC/hash/checksum strings, per-file IDs, and changing technical suffixes such as `FLAC` versus `FLACx2` with `{hash}`, `{crc}`, `{audio}`, or `{a}`. Do not copy one file's hash or audio suffix into a rule meant to match the whole group.
...
- `Complete Edition`, `Compilation`, `Omnibus`, and `Remix` can indicate edited or compilation forms; do not assume one-to-one TV episode mapping without evidence.
...
- Travel/location extras: `Travel`, `Tour`, `Journey`, `Location`, `Location Hunting`, `旅`, `ロケ`, `出張`. These are often live-action companion features; map them only when Bangumi exposes a concrete anime subject or episode for that exact item.
...
- If the subject identity is supportable and a compact rule can pass validation, do not continue researching just to make the notes longer.
