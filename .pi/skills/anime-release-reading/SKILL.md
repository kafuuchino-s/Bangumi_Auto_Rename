...
3. Infer groups from relative folders, repeated title prefixes, season/movie/OVA/SP words, and episode-number runs. A group can be a TV season, a movie, an OVA/OAD entry, a special set, or supplemental extras. If selector construction is slowing the recipe, use `get_local_recipe_params_scaffold`; it copies local selector/range stubs but does not choose target semantics.
...
- Numbered special files such as `SP01`, `SP02`, `Special 1`, or `S00E01` are candidate special entries, not automatic extras. Check the selected same-season Bangumi subject for matching special rows by sort/order before mapping them; if current bounded evidence exposes no compatible rows, keep the supportable mapping and validate the affected SP/bonus group as supplemental or unresolved for verifier-guided repair.
...
- Before the first validation, a useful evidence budget is representative lookup/search for active groups, one bounded relation graph when it helps, and episode lists/windows for subjects you plan to use. `run_progress` can reveal whether you are still in evidence collection and whether verifier feedback exists; it is a self-check fact surface, not a fixed-layer instruction.
- Treat params validation as a trial check of a compact grouping, not as the final rename decision. The first trial check does not need to be accepted or warning-free. An imperfect first check can expose selector gaps, duplicate targets, missing episode rows, and review warnings that are easier to repair than by continuing open-ended title research.
- Validate a grouped recipe once the supportable groups have plausible targets and the remaining groups have a conservative mapped or supplemental disposition. If validation reports missing target episodes for one group, repair that group using targeted evidence instead of restarting the whole investigation.
- If one short or bonus-like group remains ambiguous after bounded evidence, a supplemental rule with a clear evidence gap is often the right compact outcome for validation. If exact Bangumi subject/episode evidence exists, map it and validate; exhaustive frontier traversal is not required before the first validation.
- If validation shows a duplicate target for split or variant local locators such as `_1`/`_2`, part markers, or version suffixes, treat it as a local packaging repair problem. Keep any distinct exposed target rows, otherwise exclude only those split/variant files from the mapped sequence and validate them as supplemental exact paths.
...
- If time is running out and a large box still has unresolved groups, finish with a tool result: validate or submit a recipe that covers all supportable groups and marks genuinely unsupported bonus-like groups with `disposition: "non_bangumi_or_supplemental"`, or fail closed with the unresolved group names.
