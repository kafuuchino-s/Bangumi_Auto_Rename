...
- 一个是`is_anime=`的输入，**可以**用`"$G"`替换，代表着创建种子时候的标签，这里如果下载的是动漫剧集，需要带上`anime`的标签，如果是真人剧集，可以带上`no`或者`real`的标签，自动整理
- 填入示例如下

```shell
curl -d "is_anime=%G&path=%F" http://127.0.0.1:5999/sendTask -f
```
