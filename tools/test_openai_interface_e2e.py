...
用法：
  python tools/test_openai_interface_e2e.py --all
  python tools/test_openai_interface_e2e.py --scenario responses_fallback
"""
...
def _build_main_success_content(*_args, **_kwargs) -> str:
    """主链路可解析 JSON 文本。"""
    return json.dumps(
        {
            "confidence": "High",
...
def _scenario_responses_supported() -> ScenarioResult:
    ...
    try:
        ...
        with _patch_methods(
            client,
            {"stream_via_responses_api": _build_main_success_content},
        ):
        ...
        with _patch_methods(
            client,
            {
                "stream_via_responses_api": _build_simple_success_content,
                "stream_via_chat_completions": _build_simple_success_content,
            },
        ):
...
def _scenario_responses_fallback() -> ScenarioResult:
    ...
    try:
        ...
        with _patch_methods(
            client,
            {
                "stream_via_responses_api": _raise_responses_unsupported,
                "stream_via_chat_completions": _build_main_success_content,
            },
        ):
            main_result = _run_main_chain(ai_client)
            assert main_result is not None
