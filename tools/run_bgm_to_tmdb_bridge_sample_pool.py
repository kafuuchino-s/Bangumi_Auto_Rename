...
def _run_result_row(
    *,
    ...
    return {
        ...
        'verified_plan_absent_count': getattr(verified_plan, 'tmdb_absent_count', 0) if verified_plan is not None else 0,
        'verified_plan_supplemental_count': getattr(verified_plan, 'supplemental_count', 0) if verified_plan is not None else 0,
...
def _summary(
    *,
    ...
    return {
        ...
        'verified_plan_absent_count_total': sum(int(row.get('verified_plan_absent_count') or 0) for row in rows),
        'verified_plan_supplemental_count_total': sum(int(row.get('verified_plan_supplemental_count') or 0) for row in rows),
