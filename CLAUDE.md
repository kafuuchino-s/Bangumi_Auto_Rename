# CLAUDE.md
Python provider client 已移除，所有语义推理由 Pi sidecar 执行
This file provides guidance to Claude Code (claude.ai/code) when working with this repository.
- 字幕 Case Agent：`subtitle_case_agent_primary_enabled` / `subtitle_case_agent_backend`（`pi` only）/ `subtitle_case_agent_pi_case_root` / `subtitle_case_agent_pi_max_turns` / `subtitle_case_agent_pi_timeout_seconds` / `subtitle_case_agent_pi_command`
## 项目概述
...
# AI 识别测试（示例）
python -m pytest tests/test_pi_healthcheck.py tests/test_case_agent_pi_runner.py -q
...
| AI 提供商 | `src/ai/` | Pi sidecar 是唯一生产 AI 执行路径；`pi_healthcheck.py` 与 `pi_api_config.py` 负责协议和门禁，旧 Python AIClient 已移除 |
| Bangumi 辅助上下文 | `src/bangumi/context_builder.py` | 只给 TV prompt 提供桥接证据，不直接决定最终季集 |
| 字幕 Case Agent | `src/subtitle/case_agent/` | Pi-only evidence-driven mapping + Verifier contract; `pi_runner.py` runs the sidecar |
| 字幕 Case Agent | `src/subtitle/case_agent/` | AI-first + evidence-driven + Verifier 合同；`local_subtitle_entry.py` 分发 pi/Pi-only；`pi_runner.py` Pi 后端 |
...
| 抓取 Case Agent | `src/subtitle/auto_fetch_case_agent/` | AI-first + evidence-driven + 轻 submit gate（candidate ranking，无 mapping 合同）；`local_auto_fetch_entry.py` 统一走 Pi 后端（Pi-only 已移除）；`pi_runner.py` Pi 后端 |
...
- 能复用现有 AI 链路时，优先复用 Pi Case Agent 入口与 `src/ai/pi_healthcheck.py`；不要重新引入 Python provider client
...
- `src/subtitle/case_agent/`：字幕 Case Agent 子系统（对齐 rename）。`local_subtitle_entry.py` 按 `subtitle_case_agent_backend` 分发 `pi`（多轮 evidence-driven）/ `Pi-only`（单轮 AI + 合同）；`pi_runner.py`+`pi_tools.py` 跑 Pi sidecar；`verifier.py` 合同校验 coverage/duplicate/accounting/合法目标视频
...
**内嵌字幕识别**（auto_fetch 扫描阶段，`auto_fetch.py`）：`_collect_videos_missing_subtitles` 判定"缺字幕"不只看外挂 sidecar，还看视频内嵌字幕轨。`_has_usable_subtitle` = 外挂命中（零成本优先，短路不探轨）OR 内嵌目标语言轨命中。内嵌探轨走独立轻量函数 `_probe_embedded_subtitle_languages`（`shutil.which('ffprobe')` + `-show_streams` 取 `codec_type=='subtitle'` 轨的 `tags.language`），**不复用** `local_fact_surface.probe_media_file`（那个只统计 count 不取语言、且 rename 链路在用改返回有风险）。ffprobe 语言码归一用独立 `EMBEDDED_LANGUAGE_MAP`（`chi`/`zho`/`zh`→`zh-CN` 保守命中、`zh-tw`/`zh-hant`→`zh-TW`、`jpn`→`ja` 等，与 `processor.LANGUAGE_MAP` 字幕组标签表口径分离）。开关 `subtitle_auto_fetch_skip_if_embedded_language`（默认 True，UI 字幕自动抓取分组可见）。鲁棒：开关关 / ffprobe 不可用 / 超时 / 无轨 / 无命中 → 回退纯外挂判定（`_has_embedded_target_language` 返 False），绝不因探轨失败漏抓。series/movie scope 两阶段：先外挂过滤收集候选，再 `_filter_by_embedded_language` 对候选 `_PROBE_CONCURRENCY=4` 并发探轨剔除内嵌命中者，保持原目录顺序；task scope 候选少串行走 `_has_usable_subtitle`。全视频被内嵌覆盖 → `missing_videos` 空 → 复用 `subtitle_already_exists` skipped。
...
- Pi endpoint：`ai_api_key` / `ai_base_url` / `ai_model` / `openai_api_interface`
- Pi protocol：`openai_api_interface`（`responses_api` / `chat_completions`）
...
- 抓取 Case Agent：`subtitle_auto_fetch_case_agent_backend`（`pi` only）/ `subtitle_auto_fetch_case_agent_pi_case_root` / `subtitle_auto_fetch_case_agent_pi_max_turns` / `subtitle_auto_fetch_case_agent_pi_timeout_seconds` / `subtitle_auto_fetch_case_agent_pi_command`
- 自动抓取：`subtitle_auto_fetch_enabled` / `subtitle_auto_fetch_provider` / `subtitle_auto_fetch_candidate_limit` / `subtitle_auto_fetch_timeout_seconds` / `subtitle_auto_fetch_browser_enabled` / `subtitle_auto_fetch_acgrip_base_url` / `subtitle_auto_fetch_preferred_language` / `subtitle_auto_fetch_skip_if_embedded_language` / `subtitle_auto_fetch_use_ai_rerank` / `subtitle_auto_fetch_search_mode` / `subtitle_auto_fetch_save_reason`
...
- 脚本：`tools/pi_ai_healthcheck.mjs`
- 模式：`manual` / `auto` / `save`
